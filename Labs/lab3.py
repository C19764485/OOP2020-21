...
class WordScramble:
    ...
    def scramble(self):
        ...
        #     scrambled_word = self.user_input[0] + self.user_input[2] + self.user_input[1] \
        #                + self.user_input[3:]
        # elif len(self.user_input) <= 3:
        #     scrambled_word = self.user_input
        # else: # here we assume this word is just one character long or the space character
        #     print("try again")
        #     scrambled_word = False
        #
        # print(scrambled_word)

        # one solution for full sentence
        scrambled_sentence = self.user_input.strip().split()

        # Get the word from the sentence
        for index, word in enumerate(scrambled_sentence):
            ...
            if len(word) > 3:
                ...
                scrambled_sentence[index] = swapped_word
            else:
                # Since the length of the word < 3 don't swap the word
                scrambled_sentence[index] = word

        # Join all the words with a space
        swapped_sentence = ' '.join(scrambled_sentence)
        # Print word
